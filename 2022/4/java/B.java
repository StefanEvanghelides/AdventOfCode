...
public class B {
    ...
    // For b) We don't need full overlap, but whether they overlap by just 1 bit.
    // This is then not overlaping, but intersection.
    public static int overlaps(int x1, int x2, int y1, int y2) {
        ...
        if (x2 >= y1 && x1 <= y2 || x1 <= y2 && x1 >= y1) return 1;
        return 0;
    }
