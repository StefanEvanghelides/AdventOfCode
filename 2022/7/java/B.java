...
public class B {
    ...
    private static final int MIN_AMOUNT = 30000000;
    private static final int MAX_AMOUNT = 70000000;
    ...
    private static class Tree {
        ...
        public Tree findSmallestChild(int diff) {
            // Choose the smallest child still greater than MIN_AMOUNT
            List<Tree> smallestChildren = new ArrayList<Tree>();
            for (Tree child : children) {
                if (child != null && child.size >= diff && child.isDir) {
                    Tree newSmallestChild = child.findSmallestChild(diff);
                    if (newSmallestChild != null)
                       smallestChildren.add(newSmallestChild);
                }
            }

            Tree smallestChild = this;
            for (Tree child : smallestChildren) {
                if (child.size < smallestChild.size)
                    smallestChild = child;
            }
    ...
    private static void solve(String[] rawInput) {
        ...
        System.out.println("Free space: " + (MAX_AMOUNT - files.size));
        System.out.println("Diff needed: " + (files.size - MAX_AMOUNT + MIN_AMOUNT));
        Tree dirToDelete = files.findSmallestChild(files.size - MAX_AMOUNT + MIN_AMOUNT);
        dirToDelete.print();
        System.out.println("Smallest child size: " + dirToDelete.size);
