...
// For part b) The trick is to use a total divisor from all monkeys!
public class B {
    ...
    private static class Monkey {
        ...
        public static long total_divisor = 1;
    ...
    private static void playRound(ArrayList<Monkey> monkeys) {
        for (Monkey monkey : monkeys) {
            ...
            for (Long item : monkey.items) {
                long val = ((Double)monkey.operation.apply(item)).longValue();
                if (val > Monkey.total_divisor)
                    val %= Monkey.total_divisor;
                int otherIdx = val % monkey.divisibleBy == 0 ? monkey.trueIdx : monkey.falseIdx;
