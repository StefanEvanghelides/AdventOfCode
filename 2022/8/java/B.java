...
public class B {
    ...
    private static class Board {
        ...
        public int highestScenicScore() {
            ...
            for (int i = 1; i < n-1; ++i) {
                for (int j = 1; j < m-1; ++j) {
                    // Now search in all 4 directions
                    int scenicScore = countFromPointBottom(i, j, b[i][j])
                                    * countFromPointTop(i, j, b[i][j])
                                    * countFromPointLeft(i, j, b[i][j])
                                    * countFromPointRight(i, j, b[i][j]);
        ...
        private int countFromPointRight(int i, int j, int val) {
            int count = 0;
            for (int k = j+1; k < m; ++k) {
                count++;
                if (b[i][k] >= val)
        ...
        private int countFromPointLeft(int i, int j, int val) {
            int count = 0;
            for (int k = j-1; k >= 0; --k) {
                count++;
                if (b[i][k] >= val)
        ...
        private int countFromPointBottom(int i, int j, int val) {
            int count = 0;
            for (int k = i+1; k < n; ++k) {
                count++;
                if (b[k][j] >= val)
        ...
        private int countFromPointTop(int i, int j, int val) {
            int count = 0;
            for (int k = i-1; k >= 0; --k) {
                count++;
                if (b[k][j] >= val)
    ...
    // O(N^3) time and O(N^2) space. For each element, about 4N extra passes.
    private static void solve(String[] rawInput) {
        ...
        System.out.println("Highest scenic score: " + b.highestScenicScore());
    }
