...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  {
    const int64_t instr = std::stoll(inp);
    instructions.push_back(instr);
  }

  if (instructions.size() < 1)
  {
    std::cout << "There are no instructions! Exitting..." << std::endl;
    return;
  }

  const uint64_t MAX_JUMPS = 100000000;

  int64_t ptr = 0;
  uint64_t count = 0;
  while(ptr >= 0 && ptr < instructions.size() && count < MAX_JUMPS)
  {
    const int64_t prev = ptr;
    ptr += instructions[ptr];
    if (instructions[prev] > 2)  instructions[prev]--;
    else instructions[prev]++;
    count++;
  }

  std::cout << "Total jumps: " << count << std::endl;
