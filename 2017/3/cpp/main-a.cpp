...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  for (const std::string& input : rawInput)
  {
    ...
    // Find the closest square, larger than or equal to the number
    int64_t i = 1;
    while (i*i < nr) i = i + 2;

    uint64_t maxDist = i - 1;
    uint64_t minDist = i / 2;

    // Now find relative placement between 0 and i.

    int64_t relativePlacement = 0;
    uint64_t j = 1;
    while (i*i - j*i + j > nr && j < 4) j++;

    relativePlacement = nr - i*i + j*i - j;

    const uint64_t dist = minDist + abs(relativePlacement - i / 2);
