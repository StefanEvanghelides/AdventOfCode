...
// For part b) run the algorithm 1B times.
constexpr uint_fast32_t MAX_ITERATIONS = 1e9;
...
void solve(const Strings& rawInput)
{
  ...
  // Implement a quick cache
  std::map<std::string, std::pair<std::string, uint_fast32_t>> cache;
  for (const std::string& rawinp : rawInput)
  {
    ...
    while (iter++ < MAX_ITERATIONS)
    {
      ...
      {
        inp = cache[inp].first;
        // This is where the smart trick starts.
        // Check how much time it took to reach this step again.
        const uint_fast32_t diff = iter - cache[inp].second + 1;
        // Now find the first iteration idx that won't repeat.
        const uint_fast32_t steps = MAX_ITERATIONS / diff - 1;
        iter += steps * diff;
        // empty cache: it won't be relevant anymore.
        cache.clear();
      }
      else
      {
        std::string startInp = inp;
        for (const std::string& cmd : commands)
        {
          applyCommand(cmd, inp);
        }
        cache[startInp] = {inp, iter};
      }
    }
  }
