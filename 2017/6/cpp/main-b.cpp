...
using States = std::map<Elements, uint64_t>;
...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  // Now check the position of the elements
  const uint64_t last = states[elements];


  std::cout << "Difference: " << iter - last << std::endl;
