...
void solve(const Strings& rawInput)
{
  ...
  // The official input is one line, but I have multiple test scenarios
  double furthestAway = -1e10;
  for (const std::string& input : rawInput)
  {
    ...
    for (const std::string& tok : tokens)
    {
      ...
      {
        std::cout << "FAULTY TOKEN! " << tok << std::endl;
      }

      // Check how far we are.
      const double steps = abs(x) + abs(y);
      if (steps > furthestAway) furthestAway = steps;
    }

    std::cout << "furthestAway: " << furthestAway << std::endl;
  }
