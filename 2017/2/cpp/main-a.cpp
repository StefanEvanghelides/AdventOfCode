...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  {
    const std::vector<std::string> elemsStr = split(line, "\t");

    std::vector<int64_t> elems;
    for (const std::string& elemStr : elemsStr)
    {
      const int64_t elem = std::stoll(trimmed(elemStr));
      elems.push_back(elem);
    }

    sort(elems.begin(), elems.end());

    checkSum += (elems.back() - elems.front());
