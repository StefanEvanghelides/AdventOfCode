...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  for (const std::string& line : rawInput)
  {
    ...
    for (const char& c : input)
    {
      ...
      else if (!isdigit(c)) decoded.push_back((char)((c - 97 + sectorID) % 26 + 97));
    }

    std::cout << decoded << "  -  " << sectorID << std::endl;
  }
}

// Input: ijmockjgz-jwezxo-nojmvbz-993[jozmb]
// Gave: northpole object storage rrl  -  993
