...
constexpr char INPUT_STRING[] {"fbgdceah"};
  ...
  else if (tokens[0][0] == 'r')
  {
    ...
    else if (tokens[0][1] == 'o')
    {
      ...
      if (tokens[1][0] == 'b')
      {
        ...
        uint_fast16_t steps = idx_it != std::string::npos ? (idx_it + 1 + (idx_it > 3)) % inp.size() : 0;
...
// For part b) I need to reverse these operations.
void applyCommandReversed(const std::string& cmd, std::string& inp)
{
  // First tokenize the command
  const Strings& tokens = split(cmd, " ");
  if (tokens.size() < 1)
  ...
  else if (tokens[0][0] == 'm')
  {
    ...
    // For b) reversed indices.
    const uint_fast32_t idx2 = std::stoul(tokens[2]);
    const uint_fast32_t idx1 = std::stoul(tokens[5]);
    const std::string c { inp[idx1] };
    inp.erase(idx1, 1);
    if (idx2 == inp.size()) inp += c;
    else inp.insert(idx2, c);
  ...
  else if (tokens[0][0] == 'r')
  {
    ...
    else if (tokens[0][1] == 'o')
    {
      ...
      if (tokens[1][0] == 'b')
      {
        ...
        const char c = tokens[6][0];
        auto idx = inp.find(c);
        uint_fast16_t steps = idx != std::string::npos ? idx/2 + (idx % 2 == 1 || idx == 0 ? 1 : 5) : 0;
        std::string newCmd = "rotate left " + std::to_string(steps) + " steps";
        applyCommand(newCmd, inp);
      }
      else
      {
        ...
        if (tokens[1][0] == 'r')
        {
          // rotate right, so now to the left.
          std::rotate(inp.begin(), inp.begin()+steps, inp.end());
        }
        else if (tokens[1][0] == 'l')
        {
          // rotate left. so now to the right.
          std::rotate(inp.begin(), inp.begin()+inp.size()-steps, inp.end());
        }
        else
        {
...
void solve(const Strings& rawInput)
{
  ...
  // For part b) reverse the order of the operations.
  Strings inputs = rawInput;
  std::reverse(inputs.begin(), inputs.end());
  for (const std::string& cmd : inputs)
  {
    applyCommandReversed(cmd, inp);
  }
  std::cout << "Final reversed string: " << inp << std::endl;
