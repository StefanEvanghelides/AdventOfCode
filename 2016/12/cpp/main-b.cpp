...
constexpr uint64_t MAX_ITERATIONS = 30000000;
...
void solve(const Strings& rawInput)
{
  ...
  // For part b) we need to initialize reg "c" to 1.
  int64_t regs[4] = {0};
  regs[2] = 1;
  uint64_t iter = 0;
  ...
  std::cout << "Value in registers: a=" << regs[0] << "  b=" << regs[1] << "  c=" << regs[2] << "  d=" << regs[3] << "  iter=" << iter << std::endl;
