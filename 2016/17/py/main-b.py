...
def find_end(curr_pos: tuple, end_pos: tuple, inp: str, curr_path: str = "") -> list[str]:
  ...
  ### For part b) we should not do this optimization!
...
def solve(raw_input):
  ...
  for inp in raw_input.split():
    ...
    # For part b) we are interested in the length of the longest path
    if len(paths) > 0:
      longest_path: str = sorted(paths, key=len, reverse=True)[0]
