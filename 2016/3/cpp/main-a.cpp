...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  {
    // parse inputs
    const std::vector<std::string>& sides = split(input, "  ");

    if (sides.size() != 3)
    {
      std::cout << "Input resulted in different sides! Input: " << input << ";  sides=" << sides.size() << std::endl;
    ...
    const uint64_t& a = std::stoull(trimmed(sides[0]));
    const uint64_t& b = std::stoull(trimmed(sides[1]));
    const uint64_t& c = std::stoull(trimmed(sides[2]));

    std::vector<uint64_t> vec {a, b, c};
    std::sort(vec.begin(), vec.end());

    if (vec[0] + vec[1] > vec[2]) count++;
  }
