...
# For part (a) we are essentially interested in all cycles of 3.
# Then we want to keep only those that have at least 1 node starting with "t".
...
def compute_cycles_per_size(lan: dict[str, set[str]], cycle_size: int) -> set[frozenset[str]]:
...
def solve(raw_input: list[str]):
	...
	# Compute cycles
	cycles_of_3: set[frozenset[str]] = compute_cycles_per_size(lan, 3)
	print_cycles(cycles_of_3)

	# Filter only relevant cycles
	count_filtered_cycles: int = filter_cycles(cycles_of_3)
	print(f"{count_filtered_cycles=}")
