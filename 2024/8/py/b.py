	...
	for antenna, antennas in antennas_map.items():
		...
		for i in range(m-1):
			for j in range(i+1,m):
				...
				# Keep multiplying them until they are out of bounds
				while in_boundaries(antinode1, n):
					antinodes.add(antinode1)

					print(f"Antinode1 in boundary: {antinode1}  {antenna1=}  {antenna2=}")

					antinode1 = (antinode1[0] + dx, antinode1[1] + dy)
				while in_boundaries(antinode2, n):
					antinodes.add(antinode2)


					print(f"Antinode2 in boundary: {antinode2}  {antenna1=}  {antenna2=}")
