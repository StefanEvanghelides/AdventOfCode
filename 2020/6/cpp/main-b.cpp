...
void solve(const Strings& rawInput)
{
  ...
  // hack, make sure we add an empty line.
  Strings input = rawInput;
  input.emplace_back();
  ...
  for (const std::string& line : input)
  {
    ...
    if (firstLine)
    {
      // add chars to the unique set of characters.
    ...
    else
    {
      ...
      std::set<char> oldChars = chars;
      chars.clear();
      for (const char& c : oldChars)
      {
