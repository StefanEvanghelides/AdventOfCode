...
void solve(const Strings& rawInput)
{
  ...
  for (int64_t i = 0; i < rawInput.size(); ++i)
  {
    const std::string& input = rawInput[i];
    ...
    const int64_t& val = std::stoll(tokens[1]);
    ...
    {
      i += val - 1;
      i = std::max(0LL, std::min(i, int64_t(rawInput.size() - 1))); // Safety guard for i.
