...
void solve(const Strings& rawInput)
{
  ...
  {
    nrs.push_back(std::stoull(input));
  }

  for (uint64_t i = 0; i < nrs.size() - 2; ++i)
  {
    bool found = false;
    for (uint64_t j = i + 1; j < nrs.size() - 1; ++j)
    {
      for (uint64_t k = j + 1; k < nrs.size(); ++k)
      {
        if (nrs[i] + nrs[j] + nrs[k] == 2020)
        {
          std::cout << "Product: " << nrs[i] * nrs[j] * nrs[k] << std::endl;
          found = true;
          break;
        }
      }
