...
void solve(const Strings& rawInput)
{
  ...
  {
    const int64_t DELTA_X = slope.first;
    const int64_t DELTA_Y = slope.second;
    int64_t x = 0, y = 0;
    uint64_t count = 0;
    for (uint64_t i = DELTA_Y; i < board.size(); i = i + DELTA_Y)
    {
      x = (x + DELTA_X) % board[i].size();
      if (board[i][x]) count++;
    }
    product *= count;
  }

  std::cout << "Total trees: " << product << std::endl;
