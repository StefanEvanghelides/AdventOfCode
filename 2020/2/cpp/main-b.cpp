...
void solve(const Strings& rawInput)
{
  ...
  for (const auto& input : rawInput)
  {
    ...
    const uint64_t& minNr = std::stoull(policyNumbers[0]) - 1;
    const uint64_t& maxNr = std::stoull(policyNumbers[1]) - 1;
    const char& policy = policyParts[1][0];

    // Now we are finally able to check the password
    if ((password[minNr] == policy) ^ (password[maxNr] == policy)) count++;
  }
