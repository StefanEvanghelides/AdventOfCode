...
bool hasBag(const std::string& bag, const std::string& MY_BAG, const std::map<std::string, std::map<std::string, uint64_t>>& bags)
{
  if (bag == MY_BAG) return true;
  if (!bags.contains(bag)) return false;

  const std::map<std::string, uint64_t>& otherBags = bags.at(bag);
  for (const auto& [b,q] : otherBags)
  {
    if (hasBag(b, MY_BAG, bags)) return true;
  }
...
void solve(const Strings& rawInput)
{
  ...
  uint64_t count = 0;
  for (const auto& [bag, otherBags] : bags)
  {
    // skip my bag
    if (bag == MY_BAG) continue;

    // Check now if the bag has it.
    count += hasBag(bag, MY_BAG, bags);
  }
