...
constexpr uint64_t MAX_TURNS = 30000000;
...
void solve(const Strings& rawInput)
{
  ...
  // NOTE: Changed the structure into a map, otherwise it's too slow!
  // Still, this took ~8s to run, perhaps it can be even better...
  ...
  for (const std::string& input : rawInput)
  {
    ...
    std::map<int64_t,std::pair<int64_t,int64_t>> nrs;
    int64_t last = 0;
    int64_t turn = 0;
    for (const auto& str : tokens)
    {
      last = std::stoll(str);
      nrs[last] = {turn,-1};
      // std::cout << " - turn: " << turn << "  last: " << last << std::endl;
      turn++;
    }
    ...
    while (turn < MAX_TURNS)
    {
      ...
      if (nrs.contains(last) && nrs[last].second > -1)
      {
        // Number seen before!
        last = nrs[last].first - nrs[last].second;
        if (nrs.contains(last)) nrs[last].second = nrs[last].first;
        else nrs[last].second = -1;
      }
      else
      {
        // New number
      ...
      // std::cout << " - turn: " << turn << "  last: " << last << "  nrs last: " << nrs[last].first << "  " << nrs[last].second << std::endl;
      turn++;
    }
    std::cout << "Last number spoken: " << last << std::endl;
  }
}
