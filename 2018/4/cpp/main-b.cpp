...
void solve(const Strings& rawInput)
{
  ...
  uint64_t global_max_m = 0;
  uint64_t global_max_mi = 0;
  uint64_t global_guard = 0;
  for (const auto& [key, val] : guards)
  {
    ...
    if (max_m > global_max_m)
    {
      global_max_m = max_m;
      global_max_mi = max_mi;
      global_guard = key;
    }
  }


  std::cout << "Total: " << global_guard * global_max_mi << std::endl;
