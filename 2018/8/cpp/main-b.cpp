...
uint64_t computeValue(const SpTree& root)
{
  if (root->children.empty()) return sumMetadata(root);

  // Children are refered based on the metadata of the node
  uint64_t value = 0;
  for (const auto& meta : root->metadata)
  {
    if (meta < 1 || meta > root->children.size()) continue;
    // Get the children
    SpTree child = root->children[meta-1];
...
void solve(const Strings& rawInput)
{
  ...
  // Part b) Compute the value
  const uint64_t value = computeValue(root);
