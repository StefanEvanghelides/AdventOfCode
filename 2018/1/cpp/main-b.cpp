...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  int64_t count = 0;
  uint64_t iter = 0;
  const std::vector<std::string> elemsStr =  split(rawInput[0], " ");
  std::vector<int64_t> nrs;
  for (const std::string& e : elemsStr)
  {
    const int64_t& nr = std::stoll(e);
    nrs.push_back(nr);
  }

  while (iter < MAX_ITERATIONS && !states.contains(count))
  {
    //std::cout << "iter=" << iter << std::endl;
    states.insert(count);
    const uint64_t idx = iter % nrs.size();
    count += nrs[idx];
    iter++;
  }
