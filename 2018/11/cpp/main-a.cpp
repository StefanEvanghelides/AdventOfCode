...
void solve(const Strings& rawInput)
{
  ...
  for (const std::string& input : rawInput)
  {
    ...
    // Now find the 3x3 cells with the maximum power.
    int64_t maxSum = 0;
    uint64_t maxX, maxY;
    for (uint64_t y = 1; y < GRID_SIZE-MASK_SIZE+1; ++y)
    {
      for (uint64_t x = 1; x < GRID_SIZE-MASK_SIZE+1; ++x)
      {
        const int64_t sum = grid[x][y] + grid[x][y+1] + grid[x][y+2] +
    ...
    std::cout << "Top-Left corner: " << maxX << "," << maxY << std::endl;
  }
