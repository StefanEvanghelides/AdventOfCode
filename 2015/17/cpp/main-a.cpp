...
uint64_t countContainers(const Numbers& containers, const uint64_t& sum)
{
  if (sum == 0) return 1;
  uint64_t total = 0;
  for (uint64_t i = 0; i < containers.size(); ++i)
  {
    ...
    total += countContainers(copyContainers, sum - containers[i]);
  }
  return total;
}
...
void solve(const Strings& rawInput)
{
  ...
  const uint64_t total = countContainers(containers, SUM);
