...
void solve(const Strings& rawInput)
{
  ...
  int64_t registers[REG_SIZE] = {0};

  // For part b) make register A start with 1.
  registers[0] = 1;

  for (uint16_t i = 0; i < rawInput.size(); ++i)
  {
    ...
    if (tokens.size() < 2)
    {
      std::cout << "Wrong input line! (2): " << inp << std::endl;
      continue;
    }
    ...
    switch (inp[0])
    {
      ...
      {
        // Increment command
        const uint16_t reg = tokens[1][0] - 'a';
        registers[reg]++;
      }
      break;
      ...
      case 'j':
      {
        // One of the jump instructions. Needs further processing.
        if (inp[1] == 'm')
        {
          // simple jump instruction.
          const int16_t& offset = std::stoi(tokens[1]);
          i += offset - 1;
        }
        else
        {
          ...
          if (tokens.size() < 3)
          {
            std::cout << "We need 3 tokens for this command! Inp: " << inp << std::endl;
            continue;
          }

          const uint16_t reg = tokens[1][0] - 'a';
          const int16_t& offset = std::stoi(tokens[2]);
          if (inp[2] == 'e' && registers[reg] % 2 == 0 ||
              inp[2] == 'o' && registers[reg] == 1)
          {
            i += offset - 1;
          }
        }
  ...
  std::cout << "Register a: " << registers[0] << ";  register b: " << registers[1] << std::endl;
