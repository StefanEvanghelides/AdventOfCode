...
void solve(const Strings& rawInput)
{
  ...
  uint64_t strChars = 0;
  uint64_t encodedChars = 0;
  for (const std::string& input : rawInput)
  {
    strChars += input.size();

    // Always add 2 chars for the 2 quotations
    encodedChars += 2;

    // Now we need a way to parse the chars.
    for (uint64_t i = 0; i < input.size(); ++i)
    {
      if (input[i] == '"' || input[i] == '\\') encodedChars++;
      encodedChars++;
    }

    // std::cout << "MemoryChars: " << memoryChars << std::endl;
  }

  std::cout << "String chars: " << strChars << ";  encoded: " << encodedChars << std::endl;
  std::cout << "Result: " << encodedChars - strChars << std::endl;
}
