...
public:
  ...
  std::unordered_map<std::string, std::vector<std::string>> getNodes() const { return nodes; };
...
private:
  ...
  {
    std::unordered_set<std::string> svec(vec.begin(), vec.end());
    return svec.size() != vec.size();
  }
  ...
  uint64_t dfs(const std::string& node = START, std::vector<std::string> visited = {})
  {
    ...
    if (nodeVisited(node, visited) && hasDuplicates(visited) ||
        nodeVisited(START, visited) && node == START)
    {
    ...
    {
      if (!nodeVisited(connection, visited) || !isSmall(connection) || !hasDuplicates(visited))
      {
...
std::ostream& operator<<(std::ostream& os, const Graph& graph)
{
  ...
  // Show first the START node.
  os << START << " -> ";
  for (const std::string& conn : nodes.at(START))
  {
    os << conn << " ";
  }
  os << std::endl;
  ...
  {
    const std::string& node = pnode.first;
    if (node == START || node == END)
    {
      continue;
    }
    const std::vector<std::string>& connections = pnode.second;
    os << node << " -> ";
  ...
  // Show lastly the END node.
  os << END << " -> ";
  for (const std::string& conn : nodes.at(END))
  {
    os << conn << " ";
  }
  os << std::endl;
