...
void prettyPrint(const std::set<std::pair<int64_t,int64_t>>& paper, const int64_t& minX, const int64_t& maxX, const int64_t& minY, const int64_t& maxY)
{
  // Move the elements into a 2D vector
  const uint64_t n = abs(maxX - minX);
  const uint64_t m = abs(maxY - minY);

  std::vector<std::vector<std::string>> vec(m);
  ...
  {
    const int64_t x = p.first - minX;
    const int64_t y = p.second - minY;
...
void solve(const std::vector<std::string>& rawInput)
{
  ...
  for (const auto& command : foldCommands)
  {
    ...
    std::set<std::pair<int64_t, int64_t>> newPaper;
    for (const auto& p : paper)
    {
      ...
      if (x > maxX)
      {
        ...
        newPaper.insert(std::make_pair(newX, y));
      }
      else if (y > maxY)
      {
        const int64_t newY = 2 * maxY - y;
        newPaper.insert(std::make_pair(x, newY));
      }
      else
      {
        newPaper.insert(std::make_pair(x, y));
      }
    }
    paper = newPaper;
  ...
  prettyPrint(paper, minX, maxX, minY, maxY);
