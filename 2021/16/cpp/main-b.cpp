...
struct State
{
  ...
  State()
    : numberStr(""), totalBits(0), sumVersions(0), value(0), firstOfTwo(true)
  {}
  ~State() = default;
};
...
State parseString(const std::string& input, const uint64_t& start, const uint64_t& size, const uint64_t& depth)
{
  ...
  if (packetTypeID == 4)
  {
    ...
    // Compute the value before returning
    if (!state.numberStr.empty())
    {
      state.value = std::stoll(state.numberStr, 0, 2);
    }
  ...
  {
    // Initialize packet values appropriately.
    // Values not mentioned here are then 0'd by default.
    switch (packetTypeID)
    {
    case 1:
      state.value = 1;
  ...
  if (startBit == '0')
  {
    ...
    int64_t length = std::stoll(lengthStr, 0, 2);
    state.totalBits = 22;
    ...
    while (length > 0)
    {
      ...
      case 1:
        ...
        break;
      case 2:
        state.value = std::min(state.value, currentState.value);
        break;
      case 3:
        state.value = std::max(state.value, currentState.value);
        break;
      case 5:
        if (state.firstOfTwo) state.value = currentState.value;
        else state.value = (uint64_t)(state.value > currentState.value);
        state.firstOfTwo = false;
        break;
      case 6:
        if (state.firstOfTwo) state.value = currentState.value;
        else state.value = (uint64_t)(state.value < currentState.value);
      ...
      // for (uint64_t i = 0; i < depth; ++i) std::cout << "  ";
      // std::cout << "Iteration Bit 0 AFTER: length=" << length << "; curr total bits: " << state.totalBits << "; sumVers: " << state.sumVersions << "; current value=" << currentState.value << "; final value=" << state.value << std::endl;
  ...
  else
  {
    ...
    const uint64_t numberSubpackets = std::stoll(numberSubpacketsStr, 0, 2);
    ...
    {
      // for (uint64_t i = 0; i < depth; ++i) std::cout << "  ";
      // std::cout << "Iteration Bit 1 BEFORE: start=" << state.totalBits << std::endl;
      ...
      // for (uint64_t i = 0; i < depth; ++i) std::cout << "  ";
      // std::cout << "Iteration Bit 1 AFTER: start=" << state.totalBits << "; current value=" << currentState.value << "; final value=" << state.value << std::endl;
