...
func unfold(pattern string, conditions []int) (string, []int) {
	unfoldedPattern := pattern + "?" + pattern + "?" + pattern + "?" + pattern + "?" + pattern
	unfoldedConditions := conditions
	for i := 0; i < 4; i++ {
		unfoldedConditions = append(unfoldedConditions, conditions...)
	}

	return unfoldedPattern, unfoldedConditions
}

// Part b) Unfold patterns and conditions.
func solve(lines []string) {
	...
	for _, line := range lines {
		...
		pattern := groups[0]
		unfoldedPattern, unfoldedConditions := unfold(pattern, conditions)
		// fmt.Println("unfolded pattern:", unfoldedPattern)
		// fmt.Println("unfolded cond:", unfoldedConditions)

		remainingUnknown := strings.Count(unfoldedPattern, "?")
		possibilities := computePossibilities(unfoldedPattern, 0, len(unfoldedPattern), remainingUnknown, unfoldedConditions)
