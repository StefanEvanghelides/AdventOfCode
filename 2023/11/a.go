...
func computeDistance(x1, y1, x2, y2 int, horizontals, verticals []int) int {
	dist := 0
	// First handle horizontals
	diff := int(math.Abs(float64(x1 - x2)))
	dist += diff
	for _, idx := range horizontals {
		// Check if the index of horizontals is between x1 and x2.
		diff1 := int(math.Abs(float64(x1 - idx)))
		diff2 := int(math.Abs(float64(x2 - idx)))
		if diff1+diff2 == diff {
	...
	for _, idx := range verticals {
		// Check if the index of horizontals is between y1 and y2.
		diff1 := int(math.Abs(float64(y1 - idx)))
		diff2 := int(math.Abs(float64(y2 - idx)))
		if diff1+diff2 == diff {
			// We got a space warp, add 1 to the distance
			dist += 1
		}
...
func solve(lines []string) {
	...
	for i := 0; i < size-1; i++ {
		for j := i + 1; j < size; j++ {
			dist := computeDistance(pointsX[i], pointsY[i], pointsX[j], pointsY[j], horizontals, verticals)
